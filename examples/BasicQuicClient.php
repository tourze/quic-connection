...
class BasicQuicClient
{
    ...
    {
        $connection->onEvent('connected', function() {
            echo "✅ 连接已建立\n";
        });
        
        $connection->onEvent('disconnected', function($conn, $errorCode, $reason) {
        ...
        $connection->onEvent('data_received', function($conn, $data) {
            echo "📨 收到数据: " . substr($data, 0, 100) . "...\n";
        });
        
        $connection->onEvent('data_sent', function($conn, $bytes) {
            echo "📤 发送数据: {$bytes} 字节\n";
        });
        
        $connection->onEvent('handshake_completed', function($conn) {
            echo "🤝 握手完成\n";
        });
        
        $connection->onEvent('stream_created', function($conn, $streamId) {
            echo "🌊 创建流: {$streamId}\n";
        });
        
        $connection->onEvent('path_changed', function($conn, $oldPath, $newPath) {
